...
var initCoreInvitation = function initCoreInvitation() {
  ...
  // tampilkan nama tamu
  var guestName = getUrlParam('to') || getUrlParam('guest') || getUrlParam('n') || (satuMomen ? satuMomen.dataset.guest : null);
  var guestNameSlot = document.getElementById("guestNameSlot");
  if (guestName && guestNameSlot) {
    guestNameSlot.innerHTML = guestName;
  }
  var barcodeGuestNameSlot = document.getElementById("barcodeGuestNameSlot");
  if (guestName && barcodeGuestNameSlot) {
    barcodeGuestNameSlot.innerHTML = guestName;
  }
  var rsvpComp = document.querySelector("rsvp-component");
  if (guestName && rsvpComp) {
    rsvpComp.setAttribute("name", guestName);
  }

  // tampilkan nama group
  var groupName = getUrlParam('group') || getUrlParam('g') || (satuMomen ? satuMomen.dataset.group : null);
  var groupNameSlot = document.getElementById("groupNameSlot");
  if (groupName && groupNameSlot) {
    groupNameSlot.innerHTML = groupName;
  }
  var barcodeGroupNameSlot = document.getElementById("barcodeGroupNameSlot");
  if (groupName && barcodeGroupNameSlot) {
    barcodeGroupNameSlot.innerHTML = groupName;
  }
  ...
  for (var i = 0; i < giftContainer.length; i++) {
    giftContainer[i].style.display = "inherit";
  }
